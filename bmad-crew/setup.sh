...
echo "==> Step 1/4: Creating new company $COMPANY_ID"
# paperclipai company create --name $COMPANY_ID
paperclipai company import . --target new --new-company-name $COMPANY_ID --include company
OUTPUT=`paperclipai company list`
PAPERCLIP_COMPANY_ID=$(echo "$OUTPUT" | sed -n 's/.*id=\([^ ]*\).*/\1/p')

echo "==> Step 2/4: Installing BMAD"
npx bmad-method install
mv _bmad-output ../_bmad-output
mv _bmad ../_bmad
mv .claude ../.claude

echo "==> Step 3/4: installing BMAD skill content from $SKILLS_REPO"
paperclipai skills import "$SKILLS_REPO" --company-id "$PAPERCLIP_COMPANY_ID"
echo

echo "==> Step 4/4: importing the crew (10 agents + hierarchy + skill references)"
paperclipai company import ./ --target existing --company-id "$PAPERCLIP_COMPANY_ID" --include agents
echo
